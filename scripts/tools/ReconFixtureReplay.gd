...
static func replay_fixture_file(path: String) -> Dictionary:
	var fixture: Dictionary = _read_json_file(path)
	if fixture.is_empty():
		return {"ok": false, "path": path, "error": "fixture_unreadable"}
	var replay: Dictionary = replay_fixture(fixture)
	replay["path"] = path
	return replay
...
static func replay_fixture_dir(dir_path: String) -> Dictionary:
	...
	for path in fixture_files:
		var result: Dictionary = replay_fixture_file(path)
...
static func replay_fixture(fixture: Dictionary) -> Dictionary:
	...
	var prior_intel: Dictionary = (fixture.get("initialPriorIntel", {}) as Dictionary).duplicate(true)
	var turn_outputs: Array[Dictionary] = []
	for turn in fixture.get("turns", []):
		if not (turn is Dictionary):
			continue
		var turn_dict: Dictionary = turn as Dictionary
		if turn_dict.has("priorIntelOverride"):
			var override_prior: Variant = turn_dict.get("priorIntelOverride", null)
			if override_prior is Dictionary:
				prior_intel = (override_prior as Dictionary).duplicate(true)
		var units: Dictionary = _units_dictionary(turn_dict.get("units", []))
		var observer_owner: int = int(turn_dict.get("observerOwner", int(fixture.get("observerOwner", 1))))
		var seed: int = int(turn_dict.get("seed", int(fixture.get("seed", 1))))
		var result: Dictionary = _resolve_once(units, observer_owner, prior_intel, seed)
		var expected: Dictionary = turn_dict.get("expected", {}) as Dictionary
	...
	for check in fixture.get("deterministicChecks", []):
		if not (check is Dictionary):
			continue
		var deterministic_result: Dictionary = _run_deterministic_check(check as Dictionary)
	...
	for progression_check in fixture.get("progressionChecks", []):
		if not (progression_check is Dictionary):
			continue
		var progression_result: Dictionary = _run_progression_check(progression_check as Dictionary)
...
static func _assert_turn_expectations(failures: Array[String], fixture_id: String, turn_id: String, expected: Dictionary, result: Dictionary) -> void:
	...
	var expected_hex_levels: Dictionary = expected.get("hexLevels", {}) as Dictionary
	for hex_id in expected_hex_levels.keys():
		if not result.has(hex_id):
			...
			continue
		var intel: Dictionary = result.get(hex_id, {}) as Dictionary
	...
	var expected_hex_level_min: Dictionary = expected.get("hexLevelMin", {}) as Dictionary
	...
	var expected_hex_level_max: Dictionary = expected.get("hexLevelMax", {}) as Dictionary
...
static func _assert_known_fields(failures: Array[String], fixture_id: String, turn_id: String, unit_id: String, expected: Dictionary, actual: Dictionary) -> void:
	for field_name in expected.keys():
		var want: Variant = expected.get(field_name)
		var got: Variant = actual.get(field_name)
		if want != got:
...
static func _unit_intel(result: Dictionary, unit_id: String) -> Variant:
	var by_id: Variant = result.get(ReconSystem.UNIT_INTEL_KEY, null)
	if not (by_id is Dictionary):
		return null
	var known: Variant = (by_id as Dictionary).get(unit_id, null)
	if known is Dictionary:
		return (known as Dictionary)
	return null

static func _run_deterministic_check(config: Dictionary) -> Dictionary:
	var units: Dictionary = _units_dictionary(config.get("units", []))
	var prior: Dictionary = (config.get("priorIntel", {}) as Dictionary).duplicate(true)
	var observer_owner: int = int(config.get("observerOwner", 1))
	var seed: int = int(config.get("seed", 1))
	var runs: int = maxi(int(config.get("runs", 2)), 2)
	var baseline: Dictionary = _resolve_once(units, observer_owner, prior, seed)
	for i in range(1, runs):
		var rerun: Dictionary = _resolve_once(units, observer_owner, prior, seed)
...
static func _run_progression_check(config: Dictionary) -> Dictionary:
	var units: Array[Dictionary] = _unit_array(config.get("adjacentFriendlies", []))
	var seed: int = int(config.get("seed", 1))
	var rng_direct: RandomNumberGenerator = RandomNumberGenerator.new()
	rng_direct.seed = seed
	var direct_gain: int = ReconSystem._roll_scout_progression(units, rng_direct)
	var manual_gain: int = _manual_progression_gain(units, seed)
	if direct_gain != manual_gain:
		return {"ok": false, "message": "manual gain mismatch direct=%d manual=%d" % [direct_gain, manual_gain]}
	var min_gain: int = int(config.get("minGain", -1))
...
static func _manual_progression_gain(units: Array[Dictionary], seed: int) -> int:
	...
	var gain: int = 0
	for unit in units:
		var denominator: int = int(ReconSystem.ROLL_DENOMINATOR_BY_TYPE.get(ReconSystem._normalized_type(unit), 0))
		if denominator <= 0:
			continue
...
static func _units_dictionary(raw_units: Variant) -> Dictionary:
	var as_array: Array = raw_units as Array
	...
	for entry in as_array:
		if not (entry is Dictionary):
			continue
		var unit: Dictionary = _normalize_unit(entry as Dictionary)
